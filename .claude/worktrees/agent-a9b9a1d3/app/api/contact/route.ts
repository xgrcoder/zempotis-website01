...
export async function POST(req: NextRequest) {
  try {
    ...
    await supabase.from("activity_log").insert([{
      event_type:  "enquiry_new",
      title:       `New enquiry from ${firstName} ${lastName ?? ""}`.trim(),
      description: `${reason} — ${company || email}`,
      metadata:    { enquiry_id: enquiry?.id, email, budget: budget ?? null },
    }]);
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 32px; background: #f0f7ff; border-radius: 12px;">
          ...
          <div style="margin-top: 20px; text-align: center;">
            <a href="${process.env.NEXT_PUBLIC_BASE_URL ?? "https://zempotis.com"}/portal/admin/enquiries" style="display: inline-block; background: linear-gradient(135deg, #2563eb, #0ea5e9); color: white; padding: 12px 24px; border-radius: 10px; text-decoration: none; font-weight: 700; font-size: 0.9rem;">
